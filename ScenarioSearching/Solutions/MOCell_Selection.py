...
def MOCell_Selection(store_dir, run_time, select_number, store_: bool = True):
    problem_type = 'MOCell'
    # select = 500
...
if __name__ == '__main__':
    ...
    # MOCell_Selection(path, 1, 1000, False)
    path = os.path.dirname(os.path.realpath(__file__))
    store = True
    # p1 = Process(target=MOCell_Selection, args=(1, 2000))
    # p2 = Process(target=MOCell_Selection, args=(1, 3000))
    # p1.start()
    # p2.start()
    #
    # aaa
    ...
    for i in range(n):
        # print(i)
        p = Process(target=MOCell_Selection, args=(path, i + 1, select_, store, ))
        p.start()
    ...
    for i in range(n):
        # print(i)
        p = Process(target=MOCell_Selection, args=(path, i + 11, select_, store, ))
        p.start()
        process_list.append(p)
    ...
    for i in range(n):
        # print(i)
        p = Process(target=MOCell_Selection, args=(path, i + 21, select_, store, ))
        p.start()
        process_list.append(p)
