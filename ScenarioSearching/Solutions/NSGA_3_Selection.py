...
def NSGA3_Selection(store_dir, run_time, select_number, store_: bool = True):
    problem_type = 'NSGA_3'
    # select = 500
    ...
    # end = time.time()
    #
    # print('NSGA3_Selection ' + str(select_number) + '_' + str(run_time) + ' running time: ', end - start)


if __name__ == '__main__':
    path = os.path.dirname(os.path.realpath(__file__))
    store = False
    # p1 = Process(target=NSGA3_Selection, args=(1, 2000))
    # p2 = Process(target=NSGA3_Selection, args=(1, 3000))
    # p1.start()
    # p2.start()
    #
    # aaa
    ...
    for i in range(n):
        # print(i)
        p = Process(target=NSGA3_Selection, args=(path, i + 1, select_, store, ))
        p.start()
    ...
    for i in range(n):
        # print(i)
        p = Process(target=NSGA3_Selection, args=(path, i + 11, select_, store, ))
        p.start()
        process_list.append(p)
    ...
    for i in range(n):
        # print(i)
        p = Process(target=NSGA3_Selection, args=(path, i + 21, select_, store, ))
        p.start()
        process_list.append(p)
